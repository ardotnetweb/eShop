...
namespace eShop.WebApplication.ServiceLayer.BaseIdentityService.Interface
{
    public interface IApplicationUserManager : IDisposable
    {
        ...
        void SeedDatabase();
        Task<UpdateProfileUserViewModel> FindUserById(int Id);
        Task<UserViewModel> FindUserByIdForShow_(int Id);
        List<ShowBaseInformationUser> GetAllPaging(string term, int page, int count);
    }
}
